...
const { isDiscountActive, applyDiscountToCheckout, buildDiscountResponse } = require('../lib/discount')
...
router.patch('/:id', async (req, res) => {
  ...
  // ── Handle discount (Day 8) ────────────────
  // Same pattern as fulfillment — only process if
  // discount capability is active in this session.
  //
  // Agent sends: { "discounts": { "codes": ["SAVE10"] } }
  //
  // Invalid code → error message in messages array
  // (NOT a 400 — same UCP pattern as missing fields)
  const { discounts } = req.body
  if (discounts?.codes && isDiscountActive(activeCaps)) {
    const result = applyDiscountToCheckout(checkout, discounts.codes)
    if (!result.success) {
      // Invalid discount code — add to messages, don't crash
      // The checkout continues — agent can try a different code
      checkout.discountError = result.message
    } else {
      checkout.discountError = null
    }
  }
  ...
  // Add discount message if invalid code was sent
  // type: 'info' — invalid discount is advisory, not blocking
  // The agent can still complete checkout without a discount
  if (checkout.discountError) {
    messages.push({
      type:    'info',
      code:    'invalid_discount',
      path:    '$.discounts.codes[0]',
      content: checkout.discountError
    })
  }
  ...
  // Build discount section — only if capability is active
  const discountSection = isDiscountActive(activeCaps)
    ? buildDiscountResponse(checkout)
    : undefined
  ...
  res.json({
    ...
    ...(discountSection    ? { discounts:   discountSection   } : {}),
