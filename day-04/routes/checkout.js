...
const { validateCheckout, SHIPPING_METHODS } = require('../lib/validate')
...
router.patch('/:id', async (req, res) => {
  ...
  // Merge shipping address from fulfillment.destinations
  if (fulfillment?.destinations?.[0]) {
    checkout.shippingAddress = {
      ...checkout.shippingAddress,
      ...fulfillment.destinations[0]
    }
  }

  // Handle shipping method selection
  if (fulfillment?.selected_option_id) {
    const method = SHIPPING_METHODS.find(
      m => m.id === fulfillment.selected_option_id
    )
    if (!method) {
      return res.status(400).json({
        code: 'invalid_shipping_method',
        message: `Shipping method '${fulfillment.selected_option_id}' does not exist`,
  ...
  // Totals change whenever:
  //   - A shipping method is selected (adds shipping cost)
  //   - A discount is applied (Day 8)
  // Always recalculate after every PATCH.
  checkout.totals = calculateTotals(checkout)

  // ── Step 4: Validate + build messages ─────
  // This is the UCP way — validate AFTER merging,
  // return messages, don't reject partial updates.
  const { messages, isComplete } = validateCheckout(checkout)

  // ── Step 5: Flip status if ready ──────────
  // The moment all required fields are present,
  // status automatically moves to ready_for_complete.
  // The agent sees this and knows it can now pay.
  if (isComplete) {
  ...
  // Build the fulfillment section for the response
  // Shows available shipping options + current selection
  ...
  res.json({
    ...
    // The messages array — the heart of Day 4
    // Empty array = no issues, checkout is ready
    messages,
    updated_at:  checkout.updated_at,
    // Only show next_steps when ready to pay
    ...(checkout.status === 'ready_for_complete' ? {
      next_steps: {
